...
function Test-FileLocks {
    ...
    if ($nodeProcesses) {
        foreach ($process in $nodeProcesses) {
            ...
            # Intentar obtener información de handles (requiere herramientas adicionales)
            try {
...
"@
    ...
    $report += "LATENCIA DE API:`n"
    $report += "===============`n"
    Test-APILatency | Out-String | Add-Content -Path $reportFile -PassThru
    
    $report += "`nUSO DE MEMORIA:`n"
    $report += "==============`n"
    Test-MemoryUsage | Out-String | Add-Content -Path $reportFile -PassThru
    
    $report += "`nCONEXIONES DE RED:`n"
    $report += "=================`n"
    Test-NetworkConnections | Out-String | Add-Content -Path $reportFile -PassThru
    
    $report += "`nRENDIMIENTO DE CPU:`n"
    $report += "==================`n"
    Test-CPUPerformance | Out-String | Add-Content -Path $reportFile -PassThru
    
    $report += "`nANÁLISIS DE LOGS:`n"
    $report += "================`n"
    Test-LogAnalysis | Out-String | Add-Content -Path $reportFile -PassThru
...
function Show-Help {
    ...
    Write-Host "  .\diagnostics.ps1 -Command latency -Samples 20 -Interval 1" -ForegroundColor Gray
    Write-Host "  .\diagnostics.ps1 -Command cpu -Samples 15" -ForegroundColor Gray
    Write-Host "  .\diagnostics.ps1 -Command report" -ForegroundColor Gray
    Write-Host ""
}
